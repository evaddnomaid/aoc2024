...
def mas_cross_count(lines):
	...
	while i < len(lines) - 1:
		...
		while j < len(lines[0]) - 1:
			...
			if mas_check(lines[i - 1][j - 1] + lines[i][j] + lines[i + 1][j + 1]) and mas_check(lines[i + 1][j - 1] + lines[i][j] + lines[i - 1][j + 1]):
				count += 1
			j += 1
		i += 1
	return count
...
def mas_check(yomas):
	print("Str to check: " + yomas)
	if (yomas == "MAS" or yomas == "SAM"):
		return True
	return False
...
testbed2 = (
	...
	"xyz")
print("Tally: " + str(tot))
print("MAS cross count: " + str(mas_cross_count(a)))
